<template>
  <div>
    <a-button type="primary" @click="showModal">找回密码</a-button>
    <a-modal
    ...
    >
      ...
      >
        <a-form-item has-feedback label="用户邮箱" name="username">
          <a-input v-model:value="formState.username" autocomplete="on"  :disabled="username_disabled"/>
        </a-form-item>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const username_disabled =ref(false)
    if (props.username!==''){
      
username_disabled.value=true
            // formState.username=prop.username
    }
    ...
    const handleFinish = (values) => {
        loading.value=true
          let check_code = localStorage.getItem('code_forget')
      let postdata = {
  ...
  if(newval===true){
   ...
   const getcode=()=>{
      user.getcode().then(res=>{
        if (res.data){
        localStorage.setItem('code_forget',res.data['code'])
